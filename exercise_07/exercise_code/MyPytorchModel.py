...
class MyPytorchModel(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        #weight_decay是正则化系数
        optim = torch.optim.Adam(self.model.parameters(), self.hparams["learning_rate"],  eps=1e-4, weight_decay=0.00030669727741074334)
        StepLR = torch.optim.lr_scheduler.StepLR(optim, step_size=1, gamma=0.8500831845176134)
...
class CIFAR10DataModule(pl.LightningDataModule):
    ...
    def prepare_data(self, stage=None, CIFAR_ROOT="../datasets/cifar10"):
        ...
        # transforms.AutoAugment() must be before transforms.ToTensor()
        train_transform = transforms.Compose([transforms.AutoAugment(), transforms.ToTensor(), transforms.Normalize(mean, std)])
        ...
        if self.opt['loading_method'] == 'Image':
            ...
            cifar_complete_augmented = torchvision.datasets.ImageFolder(root=CIFAR_ROOT, transform=train_transform)
            cifar_complete_val_test = torchvision.datasets.ImageFolder(root=CIFAR_ROOT, transform=val_test_transform)
            ...
            self.dataset["train"], self.dataset["val"], self.dataset["test"] = cifar_complete_augmented,\
                cifar_complete_val_test, cifar_complete_val_test
