...
def batchnorm_backward(dout, cache):
    ...
    dx_norm = gamma*dout
    dvar = np.sum(dx_norm*x_norm, axis=0)*(-1/2.)*(1./(var+eps))
    dmean=np.sum(dx_norm, axis=0)*(-1)/np.sqrt(var + eps) + dvar*np.sum((-2)*x_minus_mean, axis=0)/N
    dx=dx_norm/np.sqrt(var + eps) + dvar*2*(x_minus_mean)/N + dmean/N
