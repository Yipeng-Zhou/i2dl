...
class Encoder(nn.Module):
    ...
    def __init__(self, hparams, input_size=28 * 28, latent_dim=20):
        ...
        self.encoder = nn.Sequential(
            nn.Linear(self.input_size, self.hparams["hidden_size"][0]),
            nn.BatchNorm1d(self.hparams["hidden_size"][0]),
            nn.ReLU(),
            #nn.Dropout(0.5),
            nn.Linear(self.hparams["hidden_size"][0], self.hparams["hidden_size"][1]),
            nn.BatchNorm1d(self.hparams["hidden_size"][1]),
            nn.ReLU(),
            #nn.Dropout(0.5),
            nn.Linear(self.hparams["hidden_size"][1], self.hparams["encoder_output_size"])
        )

        nn.init.kaiming_normal_(self.encoder[0].weight, nonlinearity='relu')
        nn.init.kaiming_normal_(self.encoder[3].weight, nonlinearity='relu')
        nn.init.kaiming_normal_(self.encoder[6].weight, nonlinearity='relu')
...
class Decoder(nn.Module):
    ...
    def __init__(self, hparams, latent_dim=20, output_size=28 * 28):
        ...
        self.decoder = nn.Sequential(
            nn.BatchNorm1d(self.hparams["encoder_output_size"]),
            nn.ReLU(),
            #nn.Dropout(0.5),
            nn.Linear(self.hparams["encoder_output_size"], self.hparams["hidden_size"][1]),
            nn.BatchNorm1d(self.hparams["hidden_size"][1]),
            nn.ReLU(),
            #nn.Dropout(0.5),
            nn.Linear(self.hparams["hidden_size"][1], self.hparams["hidden_size"][0]),
            nn.BatchNorm1d(self.hparams["hidden_size"][0]),
            nn.ReLU(),
            #nn.Dropout(0.5),
            nn.Linear(self.hparams["hidden_size"][0], self.output_size)
        )

        nn.init.kaiming_normal_(self.decoder[2].weight, nonlinearity='relu')
        nn.init.kaiming_normal_(self.decoder[5].weight, nonlinearity='relu')
...
class Autoencoder(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        StepLR = {'scheduler': torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer=optim, mode='min', 
                                                                    factor=self.hparams["lr_decay"], patience=2), 'monitor': 'train_loss_ae'}
...
class Classifier(pl.LightningModule):
    ...
    def __init__(self, hparams, encoder, train_set=None, val_set=None, test_set=None):
        ...
        self.model = nn.Sequential(
            ...
            nn.ReLU(),
            #nn.Dropout(0.5),
            nn.Linear(self.hparams["encoder_output_size"], self.hparams["hidden_size"][2]),
            nn.BatchNorm1d(self.hparams["hidden_size"][2]),
            nn.ReLU(),
            #nn.Dropout(0.5),
            nn.Linear(self.hparams["hidden_size"][2], self.hparams["classifier_output_size"])
        )
    ...
    def configure_optimizers(self):
        ...
        optim = torch.optim.Adam(params, lr=self.hparams["learning_rate"],
                                                                    weight_decay=self.hparams["weight_decay"])
        # StepLR = {'scheduler': torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer=optim, mode='min', 
        #                                                             factor=self.hparams["lr_decay"], patience=2), 'monitor': 'train_loss_cls'}
